...
def create_quote_card(quote, author, context=""):
    """명언 카드 생성"""
    context_html = f'<div style="color: var(--text-light); font-size: 0.9rem; margin-top: 0.5rem;">{context}</div>' if context else ''
    html = f'''
...
def create_mirror_coaching_card(title, insights, questions):
    """AI 거울 코칭 결과를 보여주는 카드 UI를 생성합니다."""
    
    insights_html = ""
    for insight in insights:
        insights_html += f'''
            <div style="display: flex; align-items: start; margin-bottom: 0.5rem; color: var(--text-secondary); line-height: 1.6;">
    ...
    for question in questions:
        questions_html += f'''
            <div style="display: flex; align-items: start; margin-bottom: 0.5rem; color: var(--text-secondary); line-height: 1.6; background: rgba(255,255,255,0.5); padding: 0.75rem; border-radius: 8px;">
                <span style="margin-right: 0.5rem;">❓</span>
    ...
    html = f'''
    <div class="card" style="background: linear-gradient(135deg, #EBF4FF 0%, #E0F2FE 100%); border: 1px solid #7DD3FC; margin-top: 1.5rem; margin-bottom: 1.5rem;">
        ...
        <div style="margin-bottom: 1.5rem;">
            <div style="color: var(--text-primary); font-weight: 600; margin-bottom: 0.75rem;">💡 AI 분석 인사이트</div>
            {insights_html}
        </div>
        
        <div>
            <div style="color: var(--text-primary); font-weight: 600; margin-bottom: 0.75rem;">🤔 성찰을 위한 질문</div>
            {questions_html}
        </div>
    </div>
    '''
...
def create_enhanced_metric_card(title: str, value: str, subtitle: str = "", tone: str | None = None):
    """
    작은 지표 카드 컴포넌트
    - title: 카드 상단 타이틀
    - value: 메인 값
    - subtitle: 보조 설명/증감
    - tone: 'positive' | 'negative' | None  -> 색상 강조
    """
    # 톤 컬러/아이콘
    if tone == "positive":
        accent = "#10B981"  # green
        ...
        accent = "var(--text-primary, #111827)"
        icon = ""

    icon_html = f"<span style='color:{accent}; font-size:0.9rem; font-weight:700;'>{icon}</span>" if icon else ""
    ...
    <div style="
        ...
        justify-content: space-between;
    ">
        <div style="font-size: 0.9rem; color: var(--text-secondary, #6b7280);">{title}</div>
        <div style="display:flex; align-items: baseline; gap: 8px;">
            <div style="font-size: 1.6rem; font-weight: 800; color: var(--text-primary, #111827); line-height: 1;">
                {value}
            </div>
            {icon_html}
        </div>
        <div style="font-size: 0.85rem; color: {accent if tone in ('positive','negative') else 'var(--text-light, #9ca3af)'};">
            {subtitle}
        </div>
    </div>
    """
